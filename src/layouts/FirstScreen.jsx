...
export default function FirstScreen({onLoadQuestions}) {
  ...
  return <>
    <div className="w-full flex flex-col my-24 items-center">
      ...
      <div className="w-full justify-center flex my-12">
          <input type="range" className="w-4/12 appearance-none bg-gray-500 self-center rounded-full" max={50} min={1} value={quizConfig.length} onChange={(event) => handleQuizConfig('length', event.target.value)}/>
      ...
      <div className="w-full justify-center flex">
        ...
        <select value={quizConfig.difficulty} onChange={(event) => handleQuizConfig('difficulty', event.target.value)} className="w-2/12 text-center font-mono font-semibold bg-transparent p-1 text-white outline-none rounded-xl border-2">
